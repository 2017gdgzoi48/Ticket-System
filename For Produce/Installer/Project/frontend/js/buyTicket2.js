...
import {Toast,NavBar,Button,Dialog,Overlay} from 'vant';
import {wsPort} from '../inc/config.js';
// import showArea from '../component/mobile/showArea.vue';
import seatMap2 from '../component/mobile/seatMap2.vue';
import buyTool from '../component/mobile/buyTool.vue';
import {io} from "socket.io-client";

let ws;
let chooseTicket=ref([]),loaded=ref(false);
...
function getByAreaid(areaid){
	return this.activity.seatMap.map.flat().filter(Boolean).filter(seat=>{
...
const app=createApp({
	data:function(){
		...
		getByAreaid=getByAreaid.bind(this);
		return {
			activity:{},
			chooseTicket,
			alreadyBuy:0,
	...
	methods:{
		...
		// 	// console.log(opr)
		// },
		addOne:async function(areaid){
			console.log('addOne!');
			let ticketList=getByAreaid(areaid),notOk=true;
			...
			let cnt=0;
			while(ticketList.length&&notOk&&(++cnt)<10){
				let seat=ticketList.sort(()=>Math.random()-0.5)[0];
				console.log(seat)
				// console.log(toId);
				let opr={
					row:seat.row,col:seat.col,aid:this.activity.aid,
					toId:sid,status:'c',kind:'choose',realPos:seat.realPos,
					price:seat.area.price,areaName:seat.area.areaName,
				}
				notOk=false;
				try{
					await wsDoOpr(ws,opr,sid,150);
				}catch(err){
					notOk=true;
				}
				ticketList=getByAreaid(areaid);
				// ticketList=[];
			}
		},
		minusOne:function(areaid){
			// console.log('-One!');
			let ticketIdx=chooseTicket.value.findIndex(t=>t.areaid==areaid);
			if(ticketIdx==-1)return;
			console.log('-One!');
			let ticket=chooseTicket.value[ticketIdx];
			let seat=this.activity.seatMap.get(ticket.row,ticket.col);
			console.log(seat);
			let opr={
				row:seat.row,col:seat.col,aid:this.activity.aid,
				toId:sid,status:'s',kind:'cancelChoose',realPos:seat.realPos,
				price:seat.area.price,areaName:seat.area.areaName,
				fuckyou:'hhhh'
			}
			wsDoOpr(ws,opr,sid).then(()=>{
				// console.log('hhh');
				// chooseTicket.value=chooseTicket.value.slice(0,ticketIdx).concat(chooseTicket.value.slice(ticketIdx+1));
			}).catch(err=>{
				Toast('取消失败');
			})
		},
		handleChange:function(oprList){
			...
			oprList.forEach(opr=>{
				...
				// console.log(col,row,aid,toId,status,realPos,sid);
				if(toId==sid){
					if(status=='c'){
						console.log(opr);
						...
						let {price,areaName,areaid}=this.activity.seatMap.map[realPos[0]][realPos[1]].area;
						if(chooseTicket.value.length+this.alreadyBuy==this.limit){
							// Toast('买的票数已经达到上限');
							console.log('卡网速？',opr);
		...
		buy:function(){
			...
			TransactionAsync(temp,[opr=>{
				...
				let {areaid,row,col,realPos}=ticket;
				let {areaName}=this.activity.seatMap.map[realPos[0]][realPos[1]].area;
				return axios.get('/api/buyTicketMode2',{
					params:{
...
app.component('m-seat-map',seatMap2);
